...
def dailyemail
  ...
  str_query_ccu = "select ccu, CONVERT_TZ(createdAt,'+00:00','-08:00') createdAt from stats where createdAt > '#{startdayutc}' and createdAt < '#{enddayutc}' AND createdAt != STR_TO_DATE('2015-01-14 08:03:57', '%Y-%m-%d %H:%i:%s') order by ccu desc"
  ...
  min_ccu = ccuArr.last
  # new player
  str_query_new_player = "select count(userId) totalnewplaying from user_v31 where createdAt >= '#{startdayutc}' and createdAt <= '#{enddayutc}' AND createdAt != STR_TO_DATE('2015-01-14 08:03:57', '%Y-%m-%d %H:%i:%s')"
  totalnewplaying = dbprod.query(str_query_new_player).fetch_hash['totalnewplaying']
  # Number of unique users
  str_query_numberuser = "select count(userId) totalplaying from user_v31 where createdAt <= '#{enddayutc}' AND createdAt != STR_TO_DATE('2015-01-14 08:03:57', '%Y-%m-%d %H:%i:%s')"
  ...
  str_query_valid_iap = "select count(status) as num_valid_iap from purchases where status = 'Valid' and createdAt >= '#{startdayutc}' and createdAt <= '#{enddayutc}' AND createdAt != STR_TO_DATE('2015-01-14 08:03:57', '%Y-%m-%d %H:%i:%s')"
  ...
  str_query_total_iap_usd = "select purchasedPearls, paidAmount, rateToUSD, userId from purchases where status = 'Valid' and createdAt >= '#{startdayutc}' and createdAt <= '#{enddayutc}' AND createdAt != STR_TO_DATE('2015-01-14 08:03:57', '%Y-%m-%d %H:%i:%s')"
  ...
  str_new = "SELECT userId FROM lumba_iap.purchases p1 WHERE status = 'Valid' AND createdAt BETWEEN '#{startdayutc}' AND '#{enddayutc}' AND NOT EXISTS (SELECT * FROM lumba_iap.purchases p2 WHERE p2.userId = p1.userId AND createdAt < '#{startdayutc}' AND createdAt != STR_TO_DATE('2015-01-14 08:03:57', '%Y-%m-%d %H:%i:%s') AND status = 'Valid')"
